...
public sealed class AppSettings
{
    ...
    {
        ["LoopOutputInterval"] = true, ["Cancellation"] = new Dictionary<string, object> { ["Timeout"] = true }
    };
    ...
    // ReSharper disable once PropertyCanBeMadeInitOnly.Global -- used by startup configuration
    [JsonPropertyName("Cancellation")] public CancellationSettings Cancellation { get; set; } = new();
    ...
    internal static bool IsValidSettingKey(string? key) => IsValidConfigKey(key);

    /// <summary>
    /// Determines whether the specified setting <paramref name="key" /> is valid based on the structure of <c>SupportedSettingKeys</c>
    /// internal dictionary.
    /// </summary>
    /// <param name="key">The setting key to validate.</param>
    ...
    /// </exception>
    /// <remarks>
    /// The key is split by dot ('.') characters, and each part is used to traverse a hierarchical structure (assumed to be nested
    /// dictionaries).
    ...
    /// <summary>
    /// Attempts to retrieve the value associated with the specified <paramref name="key" /> from the given
    /// <paramref name="currentEntry" />
    /// if it is a <see cref="Dictionary{TKey, TValue}" /> with string keys and object values.
    /// </summary>
    /// <param name="currentEntry">
    /// The object expected to be a <see cref="Dictionary{String, Object}" /> from which to retrieve the
    /// value.
    /// </param>
